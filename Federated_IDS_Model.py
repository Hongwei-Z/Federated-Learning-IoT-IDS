...
def make_dirs_for_results(dataset_name, model_name, is_federated=True):
    mode = "Federated" if is_federated else "Centralized"
...
def create_cnn_model(input_dim: int, num_classes: int, params: dict) -> Sequential:
    ...
    with tf.device(device_name):
        ...
        model = Sequential([
            ...
            layers.Conv1D(filters=params.get("filters", 64), kernel_size=params.get("kernel_size", 3),
                          activation="relu"),
            layers.MaxPooling1D(pool_size=params.get("pool_size", 2)),
            layers.Conv1D(filters=params.get("filters_2", 128), kernel_size=params.get("kernel_size", 3),
                          activation="relu"),
            layers.MaxPooling1D(pool_size=params.get("pool_size", 2)),
            layers.Flatten(),
...
def run_federated_training(model_type, client_data, input_dim, num_classes,
    ...
    if strategy.final_parameters is None:
        logging.warning("Failed to get global model parameters!")
        return None
...
# ============ Plotting and Report Saving ============
def plot_metrics(train_metrics, val_metrics, dataset_name, model_name, save_dir):
...
def plot_confusion_matrix(cm, dataset_name, model_name, save_dir, title_extra=""):
...
# ============ Main Function ============
def main():
    parser = argparse.ArgumentParser(description="Federated & Centralized IDS Demo")
    parser.add_argument("--datasets", nargs="+", default=["multi.csv"], help="Training Set File")
    parser.add_argument("--federated_rounds", type=int, default=50, help="Rounds")
    parser.add_argument("--local_epochs", type=int, default=0, help="Epochs")
    parser.add_argument("--model_params", type=str, default="", help="Model Parameters")
    ...
    if args.model_params:
        try:
            ...
            logging.warning("Failed to parse model_params, use default configuration")

    ALL_MODEL_TYPES = ["cnn", "mlp", "transformer", "random_forest", "decision_tree"]

    for dataset_csv in args.datasets:
        if not os.path.exists(dataset_csv):
            logging.warning(f"Dataset file {dataset_csv} not exist!")
            continue
        logging.info(f"================ Dataset Loading: {dataset_csv} ================")
        ...
        # Federated training (Keras models only)
        for model_type in ALL_MODEL_TYPES:

            if model_type in ["random_forest", "decision_tree"]:
                logging.info(f"[Federated - Skip] model={model_type}")
                ...
                logging.info(f"  -> Average accuracy: {total_acc / total_samples if total_samples else 0:.4f}")
                continue

            else:
                logging.info(f"[Federated Training] model={model_type}, Rounds={args.federated_rounds}")
                GLOBAL_TRAIN_METRICS[model_type] = {"round": [], "loss": [], "accuracy": []}
                GLOBAL_VAL_METRICS[model_type] = {"round": [], "loss": [], "accuracy": []}

                # Get model-specific parameter configurations (if any), otherwise use the defaults
                ...
                if global_model is not None:
                    ...
                    if GLOBAL_TRAIN_METRICS.get(model_type) and GLOBAL_VAL_METRICS.get(model_type):
                        plot_metrics(GLOBAL_TRAIN_METRICS[model_type], GLOBAL_VAL_METRICS[model_type],
                    ...
                    logging.info(f"  -> Global model evaluation: Loss={final_loss:.4f}, Acc={final_acc:.4f}")
                    y_pred = np.argmax(global_model.predict(X_eval), axis=1)
                    cm = confusion_matrix(y, y_pred)

                    plot_confusion_matrix(cm, os.path.basename(dataset_csv), model_type, fed_save_dir,
                                          title_extra="Fed")
                    save_classification_report(y, y_pred, os.path.basename(dataset_csv), model_type, fed_save_dir,
                                               title_extra="Fed")

        # Centralized Training
        logging.info(f"[Centralized Training] Dataset {dataset_csv}, Samples: {len(X)}")
        ...
        for model_type in ALL_MODEL_TYPES:
            ...
            if model_type in ["cnn", "rnn", "mlp", "transformer"]:
                ...
                logging.info(f"    - validation: Loss={final_loss:.4f}, Acc={final_acc:.4f}")
                ...
                plot_confusion_matrix(cm, os.path.basename(dataset_csv), model_type, local_save_dir,
                                      title_extra="Local")
                save_classification_report(y_val, y_pred, os.path.basename(dataset_csv), model_type, local_save_dir,
                                           title_extra="Local")
                ...
                logging.warning(f"Unknown model type: {model_type}, skip!")
